...
run "dump-knowledge"    uv run f5kb dump --all --out "$DUMP" --types=Knowledge --limit=3 --config ../config.yaml
run "dump-ls"           ls "$DUMP/Knowledge/"
run "dump-index"        bash -c "python3 -m json.tool '$DUMP/_index.json'"
run "dump-days"         uv run f5kb dump --days=7 --out "$DUMP" --types=Support_Solution --limit=5 --config ../config.yaml

# ── §7  enrich [network] ─────────────────────────────────────────────────────

section "§7  enrich [network]"
run "dump-bug"          uv run f5kb dump --all --out "$DUMP" --types=Bug_Tracker --limit=3 --config ../config.yaml --yes
...
run "sync-dry"          uv run f5kb sync --days=3 --out "$SYNC_DUMP" --db "$DB_SYNC" --dry-run --types=Knowledge --limit=5 --config ../config.yaml
run "sync-real"         uv run f5kb sync --days=3 --out "$SYNC_DUMP" --db "$DB_SYNC" --yes --types=Knowledge --limit=5 --no-enrich --config ../config.yaml
run "sync-since-last"   uv run f5kb sync --since-last-run --out "$SYNC_DUMP" --db "$DB_SYNC" --types=Knowledge --no-enrich --limit=5 --config ../config.yaml

# ── §9  reconcile [network] ──────────────────────────────────────────────────

section "§9  reconcile [network]"
run "reconcile-report"  uv run f5kb reconcile --dump "$SYNC_DUMP" --db "$DB_SYNC" --types=Knowledge --config ../config.yaml
run "reconcile-json"    uv run f5kb reconcile --dump "$SYNC_DUMP" --db "$DB_SYNC" --types=Knowledge --json --config ../config.yaml
...
run "recent-multi"  uv run f5kb recent --days=7 --out /tmp/f5kb_testrun_recent --types=Knowledge,Support_Solution --limit=5 --config ../config.yaml
run "recent-ls"     ls /tmp/f5kb_testrun_recent/
run "recent-index"  bash -c "python3 -m json.tool /tmp/f5kb_testrun_recent/_index.json"
run "recent-1day"   uv run f5kb recent --days=1 --out /tmp/f5kb_testrun_recent2 --config ../config.yaml
...
if [[ "${SKIP_DISCOVER:-}" == "1" ]]; then
    ...
    run "discover-yaml"  uv run f5kb discover --out /tmp/f5kb_testrun_products.yaml --config ../config.yaml
    run "discover-json"  uv run f5kb discover --format=json --out /tmp/f5kb_testrun_products.json --config ../config.yaml
...
expect_fail "err-dump-no-mode"    uv run f5kb dump --out /tmp/x --config ../config.yaml
expect_fail "err-bad-subcommand"  uv run f5kb not-a-command
expect_fail "err-sync-conflict"   uv run f5kb sync --all --days=3 --out /tmp/x --config ../config.yaml
